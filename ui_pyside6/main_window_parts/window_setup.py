...
class WindowSetupMixin:
    ...
    def setup_window(self):
        ...
        self.setWindowTitle("Magic Time Studio v3.0 - PySide6")
    ...
    def create_status_bar(self):
        ...
        from PySide6.QtWidgets import QProgressBar, QLabel, QHBoxLayout, QWidget
    ...
    def closeEvent(self, event):
        """Handle venster sluiten - veilige cleanup van threads"""
        try:
            print(f"🚪 [CLOSE] Hoofdvenster wordt gesloten, start cleanup...")
            
            # Zoek naar actieve processing threads en voer cleanup uit
            if hasattr(self, 'processing_panel') and self.processing_panel:
                if hasattr(self.processing_panel, 'processing_core') and self.processing_panel.processing_core:
                    if hasattr(self.processing_panel.processing_core, 'processing_thread') and self.processing_panel.processing_core.processing_thread:
                        thread = self.processing_panel.processing_core.processing_thread
                        if thread and thread.isRunning():
                            print(f"🧹 [CLEANUP] Stoppen van actieve processing thread...")
                            thread.cleanup()
            
            # Wacht even om threads tijd te geven om te stoppen
            import time
            time.sleep(0.5)
            
            print(f"✅ [CLOSE] Cleanup voltooid, venster kan sluiten")
            event.accept()
            
        except Exception as e:
            print(f"⚠️ [WAARSCHUWING] Fout tijdens cleanup: {e}")
            event.accept()  # Accepteer sluiten ook bij fout