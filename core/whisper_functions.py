...
def transcribe_audio_whisperx(audio_path: str, model_name: str = "large-v3", 
                              ...
                              word_level: bool = True) -> Optional[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        word_level: Schakel word-level alignment in voor betere accuracy
    ...
    try:
        ...
        # Word-level alignment voor betere timing accuracy
        word_alignments = None
        if word_level:
            try:
                # Probeer word-level alignment toe te voegen
                result = model.align(result["segments"], result["language"], audio_path)
                word_alignments = result.get("word_segments", [])
                logger.info("Word-level alignment toegevoegd voor betere timing accuracy")
            except Exception as e:
                logger.warning(f"Word-level alignment gefaald: {e}, gebruik standaard timing")
        ...
        for segment in result["segments"]:
            transcriptions.append({
                ...
                "words": segment.get("words", [])  # Word-level timing info
            })
        ...
        result = {
            ...
            "word_level_available": word_alignments is not None
        }
...
def create_whisperx_srt_file(transcriptions: List[Dict[str, Any]], 
                            output_path: str,
                            word_alignments: Optional[List[Dict[str, Any]]] = None,
                            enhanced: bool = False) -> bool:
    """
    Maak WhisperX SRT bestand met word-level alignment
    ...
    """
    try:
        from .subtitle_functions import create_whisperx_srt_content
        return create_whisperx_srt_content(
            transcriptions, 
            output_path, 
            word_alignments, 
            enhanced
        ...
        logger.error("Subtitle functies niet beschikbaar")
        return False
    except Exception as e:
        logger.error(f"Fout bij maken WhisperX SRT bestand: {e}")
        return False
...
def is_whisperx_srt_available() -> bool:
    ...
    """
    try:
        from .subtitle_functions import is_whisperx_srt_available
        return is_whisperx_srt_available()
    except ImportError:
        return False
