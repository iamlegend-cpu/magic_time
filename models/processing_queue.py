...
class ProcessingQueue:
    ...
    def add_video(self, video_path: str, settings: Dict[str, Any]) -> None:
        ...
        logger.debug(f"📋 Video toegevoegd aan queue: {os.path.basename(video_path)}")
    
    def process_all(self) -> None:
        """Verwerk alle videos in de wachtrij"""
        if not self.queue:
            logger.debug("📋 Queue is leeg")
            return
        ...
        self.queue.clear()
        self.processing = False
        logger.debug("✅ Alle videos verwerkt!")
    
    def clear_queue(self) -> None:
        """Leeg de wachtrij"""
        self.queue.clear()
        logger.debug("📋 Wachtrij geleegd")
